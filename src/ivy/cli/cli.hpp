...
public:
    ...
    Command(list_t<const char *, Alloc> &args)
        : _args(args), _description(nullptr), _callback(nullptr)
    {
    }

    Command(list_t<const char *, Alloc> &args,
            const char *name)
        : _args(args), _description(nullptr)
    {
...
public:
    Flag(list_t<const char *, Alloc> &args,
         const char *name)
        : _args(args), _description(nullptr), _set(false)
    {
        _aliases.push_back(name);
    }
...
public:
    Option(list_t<const char *, Alloc> &args,
           ...
           const char *name)
        : _args(args), _errors(errors), _defaultValue(nullptr), _description(nullptr), _required(false), _set(false)
    {
        _aliases.push_back(name);
    }
...
public:
    ...
    BasicParser(int argc, const char **argv, std::ostream &helpStream = std::cout, std::ostream &errorStream = std::cerr)
        : _defaultCommand(_args), _hasDefaultCommand(false), _helpStream(helpStream), _errorStream(errorStream)
    {
...
private:
    ...
    BasicParser(BasicParser<Alloc> &other)
        : _defaultCommand(_args), _hasDefaultCommand(false), _helpStream(other._helpStream), _errorStream(other._errorStream)
    {
        bool dashEncountered = false;
